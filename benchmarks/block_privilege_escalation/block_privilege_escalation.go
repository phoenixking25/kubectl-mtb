...
func init() {
	err := BPEbenchmark.ReadConfig("/home/phoenix/GO/src/github.com/phoenixking25/kubectl-mtb/benchmarks/block_privilege_escalation/config.yaml")
	if err != nil {
		fmt.Println(err.Error())
	}
}

var BPEbenchmark = &benchmark.Benchmark{
	Run: func(tenant string, tenantNamespace string, kclient, tclient *kubernetes.Clientset) (bool, error) {
		podSpec := &util.PodSpec{NS: tenantNamespace, AllowPrivilegeEscalation: true}
		...
		if err == nil {
			return false, fmt.Errorf("Tenant must be unable to create pod that sets AllowPrivilegeEscalation to true")
		}
		fmt.Println(err.Error())
		return true, nil
	},
